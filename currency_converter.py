...
def exchange():
    ...
    try:
        ...
        e_btc.insert(0, round((float(e_uah.get()) / float(JSON_object[0]["sale"])) / float(JSON_object[2]["sale"]), 2))
    except:
        messagebox.showwarning("Warning", "Check the amount entered")
...
try:
    ...
    messagebox.showerror("Error", "Error in obtaining exchange rates")
...
btc_currency.grid(row=3, column=0, sticky=EW)
btc_buy = Label(header_frame, text=JSON_object[2]["buy"], font="Arial 10")
btc_buy.grid(row=3, column=1, sticky=EW)
btc_sale = Label(header_frame, text=JSON_object[2]["sale"], font="Arial 10")
btc_sale.grid(row=3, column=2, sticky=EW)
...
res_frame.pack(expand=True, fill=BOTH, pady=5)
...
# BTC
l_btc = Label(res_frame, text="BTC:", font="Arial 10 bold")
l_btc.grid(row=4, column=0)
e_btc = ttk.Entry(res_frame, justify=CENTER, font="Arial 10")
e_btc.grid(row=4, column=1, columnspan=2, padx=10, sticky=EW)
e_btc.insert(0, round((START_AMOUNT / float(JSON_object[0]["sale"])) / float(JSON_object[2]["sale"]), 2))
